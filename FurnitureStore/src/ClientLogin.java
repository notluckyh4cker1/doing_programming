...
public class ClientLogin {
    public static void openClientLogin() {
        JFrame loginFrame = new JFrame("Вход клиента");
        ...
        loginButton.addActionListener(e -> {
            ...
            if (DataStorage.getClientData().containsKey(phone)) {
                User user = DataStorage.getClientData().get(phone); // Получаем объект данных пользователя
                String storedPassword = user.getPassword(); // Получаем сохранённый пароль
                if (storedPassword.equals(password)) {
                    String firstName = user.getFirstName(); // Извлекаем имя клиента
